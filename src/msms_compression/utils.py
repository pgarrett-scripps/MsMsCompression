...
mz_array = np.array([p[0] for p in DEFAULT_SPECTRA], dtype=np.float32)

print(len(mz_array))

encoded_mz = delta_encode(mz_array)
print(f'Encoded len: {len(encoded_mz)}, '
      f'Original len: {len("".join([str(p[0]) for p in DEFAULT_SPECTRA]))}, '
      f'Original len (float32): {len("".join([str(mz) for mz in mz_array]))}')

decoded_mz = list(delta_decode(encoded_mz))
print([round(mz, 4) for mz in list(decoded_mz)[:5] + list(decoded_mz)[-5:]])
print([round(mz, 4) for mz in list(mz_array)[:5] + list(mz_array)[-5:]])
print()

intensity_array = np.array(sorted([p[1] for p in DEFAULT_SPECTRA]), dtype=np.float32)

encoded_mz = delta_encode(intensity_array)
print(f'Encoded len: {len(encoded_mz)}, '
      f'Original len: {len("".join([str(p[0]) for p in DEFAULT_SPECTRA]))}, '
      f'Original len (float32): {len("".join([str(mz) for mz in mz_array]))}')
...
decoded_mz = list(delta_decode(encoded_mz))
print([round(mz, 4) for mz in list(decoded_mz)[:5] + list(decoded_mz)[-5:]])
print([round(mz, 4) for mz in list(intensity_array)[:5] + list(intensity_array)[-5:]])
print()

    int_array = np.array([p[1] for p in DEFAULT_SPECTRA], dtype=np.float32)
    
    encoded_int = hex_encode(int_array)
    print(f'Encoded len: {len(encoded_int)}, '
          f'Original len: {len("".join([str(p[1]) for p in DEFAULT_SPECTRA]))}, '
          f'Original len (float32): {len("".join([str(intensity) for intensity in int_array]))}')
    
    compressed_spectrum = url_encode_spectrum(mz_array, int_array)
    print(compressed_spectrum)
    print(f'Compressed len: {len(compressed_spectrum)}, '
          f'Original len: {len("".join([str(p[0]) for p in DEFAULT_SPECTRA])) + len("".join([str(p[1]) for p in DEFAULT_SPECTRA]))}, '
          f'Original len (float32): {len("".join([str(mz) for mz in mz_array])) + len("".join([str(intensity) for intensity in int_array]))}')
    
    dumb_compressed_spectrum = dumb_encode_spectrum(mz_array, int_array)
    print(f'Compressed len: {len(dumb_compressed_spectrum)}, '
          f'Original len: {len("".join([str(p[0]) for p in DEFAULT_SPECTRA])) + len("".join([str(p[1]) for p in DEFAULT_SPECTRA]))}, '
          f'Original len (float32): {len("".join([str(mz) for mz in mz_array])) + len("".join([str(intensity) for intensity in int_array]))}')
    
    # calculate the percent difference between the mz and intensity values
"""